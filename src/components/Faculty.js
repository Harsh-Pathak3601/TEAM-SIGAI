...
const LinkedInIcon = () => (
  <svg
    ...
    viewBox="0 0 24 24"
    fill="currentColor"
    className="text-gray-400 hover:text-blue-300 transition-colors cursor-pointer"  // ← added cursor-pointer
  >
  ...
  <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 md:p-8 shadow-2xl shadow-blue-500/20 w-full max-w-5xl mx-auto backdrop-blur-sm bg-opacity-80 h-full cursor-default">
    ...
    */}
    ...
    <div className="flex flex-col md:flex-row gap-8 items-center md:items-start h-full cursor-default">
      ...
      <div className="flex-shrink-0 flex flex-col items-center md:items-start text-center md:text-left cursor-default">
        <img
          src={imageUrl}
          alt={name}
          className="w-52 h-72 sm:w-60 sm:h-80 md:w-72 md:h-96 lg:w-80 lg:h-[28rem] object-cover border-4 border-gray-700 shadow-lg"
      ...
      <div className="flex-1 flex flex-col cursor-text">
        ...
        <p className="text-sm md:text-base text-gray-300 leading-relaxed flex-grow text-justify cursor-text">
          {description}
        </p>
      </div>
...
export default function App() {
  ...
  return (
    <div
      className="bg-gray-950 p-4 md:p-12 text-gray-100 font-sans cursor-default"
      style={{ background: 'radial-gradient(circle, #1a202c 0%, #0a0e1a 100%)' }}
    >
